"""Hourly forecasts for NOAA Weather."""
...
class HourlyForecast:
    ...
    def _parse_period(self, period: dict[str, any]):
        ...
        try:
            value = float(
                period.get(const.NOAA_ATTR_NATIVE_DEW_POINT).get(const.META_VALUE)
            )
            unit = period.get(const.NOAA_ATTR_NATIVE_DEW_POINT).get(const.META_UNIT)
            if const.NOAA_UNIT_DEG_C not in unit:
                value = noaa_utils.convert_F_to_C(value)
            forecast[const.FORECAST_ATTR_NATIVE_DEW_POINT] = value
        except (ValueError, TypeError):
            forecast[const.FORECAST_ATTR_NATIVE_DEW_POINT] = None
        try:
            forecast[const.FORECAST_ATTR_HUMIDITY] = float(
                period.get(const.NOAA_ATTR_HUMIDITY).get(const.META_VALUE)
            )
        except (ValueError, TypeError):
